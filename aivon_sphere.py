...
from sphere.telegram_bot import TelegramBotThread  # noqa: E402
from sphere.media import (  # noqa: E402
    SphereMediaMixin,
    SpotifyController as SpotifyController,
    SpotifyControlThread as SpotifyControlThread,
    SearchThread as SearchThread,
)
from sphere.ui import SphereUIMixin, HandGestureThread as HandGestureThread  # noqa: E402
from sphere.config import (  # noqa: E402
    load_sphere_config, save_sphere_config,
    load_config, save_config,
    DEFAULT_CONFIG, load_commands,
)
from sphere.sound import JarvisSound  # noqa: E402
from sphere.launcher import AppLauncher  # noqa: E402
...
# │  TelegramBotThread: moved to sphere/telegram_bot.py                         │
# │  Re-exported above via: from sphere.telegram_bot import TelegramBotThread   │
# └────────────────────────────────────────────────────────────────────────────┘
# TelegramBotThread — full implementation moved to sphere/telegram_bot.py (imported above).
_TelegramBotThread_PLACEHOLDER = TelegramBotThread  # re-export alias for backward compatibility
# TelegramBotThread body — moved to sphere/telegram_bot.py (imported above).
# Original 987 lines of class body removed. See sphere/telegram_bot.py.
...
# └────────────────────────────────────────────────────────────────────────────┘
# Config functions moved to sphere/config.py — imported at top of file.
# Stubs below keep the module namespace intact during refactor.
SPHERE_CONFIG_DEFAULTS = DEFAULT_CONFIG  # backward-compat alias


# load_config body — moved to sphere/config.py (imported at top of file).


# ═══════════════════════════════════════════════════════════
# SPHERE CONFIG — moved to sphere/config.py
# ═══════════════════════════════════════════════════════════
# SPHERE_CONFIG_DEFAULTS, load_sphere_config, save_sphere_config,
# load_commands — all moved to sphere/config.py (imported at top of file).
# Backward-compat alias: SPHERE_CONFIG_DEFAULTS already set to DEFAULT_CONFIG above.


# load_sphere_config — stub (real implementation in sphere/config.py, imported above)
# save_sphere_config — stub (real implementation in sphere/config.py, imported above)
# load_commands      — stub (real implementation in sphere/config.py, imported above)
...
# └────────────────────────────────────────────────────────────────────────────┘
class AivonSphere(SphereTTSMixin, SphereSystemMixin, SphereProductivityMixin, SphereCommandsMixin, SphereMemoryMixin, SphereNetworkMixin, SphereAutomationMixin, SphereAIMixin, SphereAudioMixin, SphereMediaMixin, SphereUIMixin, QWidget):
...
"""
    ...
    # _handle_music, _handle_steam, _handle_media, _handle_add_media,
    # _on_work_started, _on_work_stopped, _execute_spotify_uri, _ensure_spotify_ctrl
    # moved to SphereMediaMixin in sphere/media.py
    # └─ sphere/media.py ─────────────────────────────────────────────────────┘
